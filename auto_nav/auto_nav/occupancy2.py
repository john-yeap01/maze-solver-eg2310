...
class Occupy(Node):
    ...
    def listener_callback(self, msg):
        ...
        #occdata = self.filter_lone_points(thresholded_data)
        ...
        #set goal to 0 as well (black dot)
        x = int(input('x'))
        y = int(input('y'))
    ...
    def filter_lone_points(self, array):
        filtered_array = array[:]
        for i in range(1, len(array) - 1):
            if array[i] != array[i - 1] and array[i] != array[i + 1]:
                filtered_array[i] = array[i - 1] if array[i - 1] == array[i + 1] else array[i + 1]
